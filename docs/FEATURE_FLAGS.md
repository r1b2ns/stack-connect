...
- **Read at:** `StackConnect/Infra/Providers/Apple/AppleAccountConnection.swift`
  ...
  - `fetchProfiles()` — when ON, calls the Rust core `Profiles.fetchProfiles()` and maps `StackCoreRust.ProvisioningProfileInfo` → `ProvisioningProfileModel` via `mapProvisioningProfileInfo` (every field maps 1:1; the raw ISO8601 `createdDate`/`expirationDate` are parsed via `parseISO8601Date`). Read only.
  - `createProfile(name:profileTypeRaw:bundleIdId:certificateIds:deviceIds:)` — when ON, passes `profileTypeRaw` STRAIGHT to the Rust core `Profiles.createProfile(name:profileType:bundleIdId:certificateIds:deviceIds:)` WITHOUT the SDK path's `ProfileType(rawValue:)` validation (the core accepts any raw string — a deliberate improvement over the SDK path, which keeps the enum validation), and maps the returned `ProvisioningProfileInfo` → `ProvisioningProfileModel` via `mapProvisioningProfileInfo` (the method returns a `CreatedProfile` wrapping the model plus the optional `profileContent`).
  - `deleteProfile(id:)` — when ON, calls the Rust core `Profiles.deleteProfile(id:)` (method is `Void`).
  - `fetchProfileContent(id:)` — when ON, calls the Rust core `Profiles.fetchProfileContent(id:)` and returns the optional `String?` content straight through. Read only.
- **Supporting types:**
