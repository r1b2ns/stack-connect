...
### Fase 4 — UI Windows com SwiftCrossUI (CONCLUÍDA, 2026-06-07)
...
> **Status de entrega — Block E (2026-06-07):** Fase 4 está **CONCLUÍDA E VALIDADA NA VM**. Home screen alcançou paridade total com iOS (render + navegação push/pop + persistência de Customize Widgets através de restart, TC-090). Todos os Blocos A–E foram mesclados em `experiment/windows`, **nenhum PR aberto**. **T-E1** (testes unitários StackHomeCore) — DONE, merged via PR #44 (commit `c1cca0d`), todas as gates passaram (56/56); **T-E2** (testes de gating SyncService) — merged via PR #45 (commit `aaad497`), StackHomeCore suite verde (51/51); **T-E3** (gate 7 GUI Home completo no `Test-WindowsPort.ps1`) — merged via PR #46 (commit `d3b79de`), sincronizado com `experiment/windows`; **T-E4** (smoke end-to-end na VM) — merged via PR #47 (commit `2e9694c`), validado na VM Windows (render, push/pop, persistência de Customize Widgets através de restart, plano de smoke test VM adicionado). O **gate 7** (`Windows GUI Home build`) do `Test-WindowsPort.ps1` compila a **Home completa** do Windows — navegação + cards de provider + banner de sync (T-B1..T-B6), container de widgets + 3 widgets + Customize Widgets (T-C1..T-C3) e o banner de alertas / loading / placeholders de navegação v1 / reflow responsivo (T-D1..T-D4) — tudo contra o `StackHomeCore` (US-010 AC-3). O build é um único `swift build` sobre o pacote `StackConnectWindowsApp` inteiro (sem filtro de produto/target), honrando `--scratch-path` (MAX_PATH, TC-091), `core.symlinks=false` e `SCUI_DEFAULT_BACKEND=WinUIBackend` (TC-092); `-RunGui` continua gateando o launch da janela (CI roda build-only). **Nota de follow-up (T-E2, merged não-blocking):** duas should-fixes do Staff Review foram mescladas unaddressed: (SF-1) o test double de Keychain `GatingInMemoryKeychain` é na verdade um stub null/no-op com nome enganoso; (SF-2) a varredura de forbidden-import em `SyncServiceGatingTests.swift` chama `ungatedImports(of: module, allowedGate: module, ...)`, muito permissivo — um `#if canImport(SwiftUI) import SwiftUI #endif` passaria, contradizendo AC-1 de US-010; deveria usar `allowedGate: ""` (strict) para os módulos proibidos.
