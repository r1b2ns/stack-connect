...
> **Status de entrega — Block E (2026-06-07):** Fase 4 está **CONCLUÍDA E VALIDADA NA VM**. Home screen alcançou paridade total com iOS (render + navegação push/pop + persistência de Customize Widgets através de restart, TC-090). Blocos A-D foram mesclados previamente; Block E: **T-E1** (testes unitários StackHomeCore) — DONE, merged em `experiment/windows` via PR #44 (commit `c1cca0d`), todas as gates passaram (56/56); **T-E2** (testes de gating SyncService) — entregue em PR #45, StackHomeCore suite verde (51/51), PR aberto aguardando revisão de close-out; **T-E3** (gate 7 GUI Home completo no `Test-WindowsPort.ps1`) — entregue em PR #46, branch sincronizado com `experiment/windows`, PR aberto aguardando re-revisão; **T-E4** (smoke end-to-end na VM) — DONE, validado na VM Windows (render, push/pop, persistência de Customize Widgets através de restart). O **gate 7** (`Windows GUI Home build`) do `Test-WindowsPort.ps1` compila a **Home completa** do Windows — navegação + cards de provider + banner de sync (T-B1..T-B6), container de widgets + 3 widgets + Customize Widgets (T-C1..T-C3) e o banner de alertas / loading / placeholders de navegação v1 / reflow responsivo (T-D1..T-D4) — tudo contra o `StackHomeCore` (US-010 AC-3). O build é um único `swift build` sobre o pacote `StackConnectWindowsApp` inteiro (sem filtro de produto/target), honrando `--scratch-path` (MAX_PATH, TC-091), `core.symlinks=false` e `SCUI_DEFAULT_BACKEND=WinUIBackend` (TC-092); `-RunGui` continua gateando o launch da janela (CI roda build-only).
