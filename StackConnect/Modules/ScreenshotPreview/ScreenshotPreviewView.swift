...
final class ScreenshotPreviewViewModel: ObservableObject {
    ...
    /// Delegates to the shared `FileArchiver` so the screenshots export and the
    /// analytics files export use the same implementation.
    nonisolated static func zipDirectory(_ source: URL) throws -> URL {
        try FileArchiver.zipDirectory(source)
    }
}
