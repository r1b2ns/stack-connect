...
/// Orchestrates background sync of accounts and their apps. Foundation-pure core
/// shared by iOS and Windows (extracted from the iOS app target in T-A9).
///
/// Per-account fetches run in parallel via TaskGroup; writes serialize through
/// the injected `PersistentStorable`. Coalesces concurrent `syncAll()` calls so
/// repeated invocations don't pile up (TC-018/TC-078).
///
/// This type is deliberately **not** an `ObservableObject` and uses no Combine
/// API (US-010 / TC-056 / TC-057). State changes are surfaced via:
/// - `state`: the latest snapshot (synchronous read).
/// - `onStateChanged`: a callback fired on every transition (MainActor).
/// - `states`: an `AsyncStream<SyncState>` that replays the current value and
///   then yields every subsequent transition.
///
/// The iOS app wraps this in a thin `ObservableObject` adapter that republishes
/// `state` via `@Published` for SwiftUI/Combine bindings.
///
/// Generic over the credentials type so core stays free of the iOS-only
/// `AppleCredentials`: iOS instantiates `SyncService<AppleCredentials>` and
/// supplies a factory that turns decoded credentials into an
/// `AppleAccountSyncing` connection backed by the App Store Connect SDK.
@MainActor
public final class SyncService<Credentials: Decodable & Sendable> {

    public typealias AppleConnectionFactory = @Sendable (Credentials) -> any AppleAccountSyncing
    ...
    public var onStateChanged: ((SyncState) -> Void)?

    /// An `AsyncStream` that immediately replays the current `state`, then yields
    /// every subsequent transition. Each access returns an independent stream.
    public var states: AsyncStream<SyncState> {
    ...
    private let sideEffects: SyncSideEffects
    private let credentialsKeyPrefix: String
    private var rootTask: Task<Void, Never>?

    public init(
        storage: PersistentStorable,
        keychain: KeyStorable,
        appleConnectionFactory: @escaping AppleConnectionFactory,
        sideEffects: SyncSideEffects = NoopSyncSideEffects(),
        credentialsKeyPrefix: String = "credentials."
    ) {
        self.storage = storage
        self.keychain = keychain
        self.appleConnectionFactory = appleConnectionFactory
        self.sideEffects = sideEffects
        self.credentialsKeyPrefix = credentialsKeyPrefix
    }
    ...
    @discardableResult
    public func syncAll(mode: SyncMode = .full) -> Task<Void, Never> {
        if let rootTask {
            SyncLog.info("[Sync] syncAll coalesced into in-flight sync")
            return rootTask
        }
    ...
    private func performSyncAll(mode: SyncMode) async {
        ...
        do {
            ...
            SyncLog.error("[Sync] Failed to load accounts: \(error.localizedDescription)")
            return
        }
        ...
        guard !accounts.isEmpty else {
            ...
            await sideEffects.syncDidFinish(mode: mode, changes: SyncChange())
            return
        }
        ...
        let prepared: [(AccountModel, (any AppleAccountSyncing)?)] = accounts.map { account in
            let creds: Credentials? = keychain.object(forKey: "\(credentialsKeyPrefix)\(account.id)")
            let connection = creds.map(appleConnectionFactory)
            return (account, connection)
        }

        SyncLog.notice("[Sync] Starting \(mode == .lightweight ? "lightweight " : "")parallel sync for \(accounts.count) Apple account(s)")
        await sideEffects.syncDidStart(mode: mode, accountCount: accounts.count)
        ...
        let changes = await withTaskGroup(of: SyncChange.self) { group -> SyncChange in
            for (account, connection) in prepared {
                group.addTask { [weak self] in
                    ...
                    let change = await Self.runAccountSync(
                        account: account,
        ...
        state.lastSyncedAt = .now
        state.isSyncing = false
        await sideEffects.syncDidFinish(mode: mode, changes: changes)

        SyncLog.notice("[Sync] syncAll completed")
    }
    ...
    private nonisolated static func runAccountSync(
        ...
        guard let connection else {
            SyncLog.error("[Sync] No credentials for \(account.name)")
        ...
        do {
            ...
            for app in finalApps {
                do {
                    ...
                    SyncLog.error("[Sync] Save failed for \(app.name): \(error.localizedDescription)")
                }
            }
            ...
            SyncLog.notice("[Sync] \(account.name): \(finalApps.count) apps, \(reviewResult.saved) reviews, \(phasedSaved) phased (\(modeLabel))")
            return change
        } catch {
            await saveMetadata(
                storage: storage,
            ...
            SyncLog.error("[Sync] \(account.name) failed: \(error.localizedDescription)")
            return SyncChange()
        }
    }
        ...
        do {
            ...
            for var review in reviews {
                ...
                do {
                    ...
                    SyncLog.error("[Sync] Save review failed for \(app.name): \(error.localizedDescription)")
                }
            }
            return (saved, newReviews)
        } catch {
            SyncLog.error("[Sync] Fetch reviews failed for \(app.name): \(error.localizedDescription)")
            return (0, [])
        }
    }
            ...
            for app in candidates {
                ...
                group.addTask {
                    do {
                        ...
                        SyncLog.error("[Sync] Phased release failed for \(app.name): \(error.localizedDescription)")
                    }
                    return false
                }
            }
