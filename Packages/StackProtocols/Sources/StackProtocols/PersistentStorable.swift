...
// requirements. They live as concrete methods on the storage implementations.
// Declaring them on the protocol with a default implementation caused calls on
// the concrete type to mis-dispatch to the default (which derives the type name
// from `T`), silently returning nothing — e.g. the widget failing to read phased
